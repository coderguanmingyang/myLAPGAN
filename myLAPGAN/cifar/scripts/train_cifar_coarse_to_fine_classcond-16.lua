...
adversarial = require 'train.double_conditional_adversarial-16'
...
opt = lapp[[
  ...
  --coarseSize       (default 8)          coarse scale
  --fineSize         (default 16)          fine scale
]]
...
opt.noiseDim = {1, opt.fineSize, opt.fineSize} --1 * 16 * 16
classes = {'0','1'}
opt.geometry = {3, opt.fineSize, opt.fineSize} --1 * 16 * 16
opt.condDim1 = 10
opt.condDim2 = {3, opt.fineSize, opt.fineSize} --3 * 16 * 16
...
if opt.network == '' then
  ...
  local sz =math.floor( ( (opt.fineSize - 5 + 1) - 5) / 2 + 1)  --floor 求比这个数小的最大整数 12
  d5 = nn.Reshape(nplanes*sz*sz)(d4)
  d6 = nn.Linear(nplanes*sz*sz, 1)(nn.Dropout()(nn.ReLU()(d5)))
  d7 = nn.Sigmoid()(d6)
  model_D16 = nn.gModule({x_d, x_c1, x_c2}, {d7})
  ...
  class1 = nn.Linear(opt.condDim1, opt.condDim2[2]*opt.condDim2[3])(g_c1) -- 10 inputs -> fine size * fine size outputs (default 16)
  ...
  g2 = nn.SpatialConvolutionUpsample(5, nplanes, 3, 3, 1)(g1)   --filter nplanes*3*3
  g3 = nn.SpatialConvolutionUpsample(nplanes, nplanes, 3, 3, 1)(nn.ReLU()(g2))
  g4 = nn.SpatialConvolutionUpsample(nplanes, 3, 3, 3, 1)(nn.ReLU()(g3))
  model_G16 = nn.gModule({x_n, g_c1, g_c2}, {g4}) --生成一个有向无环图
...
parameters_D,gradParameters_D = model_D16:getParameters()
parameters_G,gradParameters_G = model_G16:getParameters()
...
cifar_scale16.init(opt.fineSize, opt.coarseSize)

-- create training set and normalize
trainData =  cifar_scale16.loadTrainSet(1, ntrain) -- start = 1 stop = 45000
...
valData =  cifar_scale16.loadTrainSet(ntrain+1, ntrain+nval)
...
function getSamples(dataset, N, perclass)
  ...
  local noise_inputs = torch.Tensor(N, opt.noiseDim[1], opt.noiseDim[2], opt.noiseDim[3]) --N*1*32*32
  local cond_inputs1 = torch.Tensor(N, opt.condDim1) --N*10
  local cond_inputs2 = torch.Tensor(N, opt.condDim2[1], opt.condDim2[2], opt.condDim2[3]) --N*3*32*32
  local gt_diff = torch.Tensor(N, opt.geometry[1], opt.geometry[2], opt.geometry[3]) --N*3*32*32
  local gt = torch.Tensor(N, 3, opt.fineSize, opt.fineSize) --N*3*32*32

  -- Generate samples
  noise_inputs:uniform(-1, 1)
  local class = 1
  local classes = {}
  --得到一系列对应类别，10个类别里每个类别都有perclass个
  for n = 1,N do
    ...
    while true do
      ...
      sample = dataset[rand]
      local max, ind = torch.max(sample[2], 1) --return the max of the label vector and the position
      if ind[1] == class then
        break
      end
    end
    cond_inputs1[n] = sample[2]:clone() --load the label vector
    cond_inputs2[n] = sample[3]:clone() --load the corase
    gt[n] = sample[4]:clone()  --load the fine
    gt_diff[n] = sample[1]:clone() --load the diff
  ...
  local preds_D = model_D16:forward({samples, cond_inputs1, cond_inputs2})

  local to_plot = {}
  for i=1,N do
    local pred = torch.add(cond_inputs2[i]:float(), samples[i]:float()) --gen
    to_plot[#to_plot+1] = gt[i]:float()           --fine
    to_plot[#to_plot+1] = pred                    --gen
    to_plot[#to_plot+1] = cond_inputs2[i]:float() --corase
    to_plot[#to_plot+1] = samples[i]:float()      --diff_gen
  end
...
while true do
  -- train/test
  adversarial.train(trainData) --one epoch
