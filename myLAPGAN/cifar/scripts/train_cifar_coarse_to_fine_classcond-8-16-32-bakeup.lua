...
getsamples = require 'sampling.getSamples'
...
opt = lapp[[
  ...
  --ifwbupsampleD     (default 0)]
  --ifwbupsampleG     (default 0)
]]
...
if opt.network8 == '' then
  ...
  g2_sc8 = nn.SpatialConvolutionUpsample(4, nplanes, 3, 3, 1)(g1_sc8)   --filter nplanes*3*3
...
if opt.network16 == '' then
  ...
  d3_sc16 = nn.SpatialConvolution(3+1, nplanes, 3, 3)(d2_sc16)
  ...
  d4_sc16 = nn.SpatialConvolution(nplanes, nplanes, 3, 3, 2, 2)(nn.ReLU()(d3_sc16))
  local sz =math.floor( ( (16 - 3 + 1) - 3) / 2 + 1)  --floor 求比这个数小的最大整数 12
  sz = 6
  ...
  g2_sc16 = nn.SpatialConvolutionUpsample(5, nplanes, 3, 3, 1)(g1_sc16)   --filter nplanes*3*3
  g3_sc16 = nn.SpatialConvolutionUpsample(nplanes, nplanes, 3, 3, 1)(nn.ReLU()(g2_sc16))
  g4_sc16 = nn.SpatialConvolutionUpsample(nplanes, 3, 3, 3, 1)(nn.ReLU()(g3_sc16))
...
if opt.network32 == '' then
 ...
 local nplanes = opt.hidden_D32
  ...
  d3_sc32 = nn.SpatialConvolution(3+1, nplanes, 5, 5)(d2_sc32)
  d4_sc32 = nn.SpatialConvolution(nplanes, nplanes, 5, 5, 2, 2)(nn.ReLU()(d3_sc32))
  ...
  g4_sc32 = nn.SpatialConvolutionUpsample(nplanes, 3, 5, 5, 1)(nn.ReLU()(g3_sc32))
...
-------------------------------------------------
-- weight upsampling
if opt.ifwbupsampleG == 1  then

  print('\n weights and bias have already upsampled \n')
  local first_filterG32  = torch.FloatTensor(64, 5, 7, 7)
  local second_filterG32 = torch.FloatTensor(64, 64, 7, 7)
  local third_filterG32  = torch.FloatTensor(3, 64, 5, 5)

  local first_filterG16  = torch.FloatTensor(64, 5, 3, 3)
  local second_filterG16 = torch.FloatTensor(64, 64, 3, 3)
  local third_filterG16  = torch.FloatTensor(3, 64, 3, 3)
  -- first conv
  for i=1,64 do
    first_filterG16[i] = pa_G16[3][i]:clone()
  end
  for i =1,64 do
    first_filterG32[i] = image.scale(first_filterG16[i], 7, 7) --3*3 -> 7*7
    model_G32:get(8).weight[i] = first_filterG32[i]:clone()
  end
  for i=1,64 do
  ...
  -- second conv
  for i=1,64 do
    second_filterG16[i] = pa_G16[5][i]:clone()
  end
  for i =1,64 do
    second_filterG32[i] = image.scale(second_filterG16[i], 7, 7) --3*3 -> 7*7
    model_G32:get(10).weight[i] = second_filterG32[i]:clone()
  end
  ...
  -- third conv
  for i =1,3 do
    third_filterG16[i] = pa_G16[7][i]:clone()
  end
  for i =1,3 do
    third_filterG32[i] = image.scale(third_filterG16[i], 5, 5) --3*3 -> 5*5
    model_G32:get(12).weight[i] = third_filterG32[i]:clone()
  end
...
if opt.ifwbupsampleD == 1  then

  print('\n weights and bias have already upsampled \n')
  local first_filterD32  = torch.FloatTensor(64, 4, 5, 5)
  local second_filterD32 = torch.FloatTensor(64, 64, 5, 5)

  local first_filterD16  = torch.FloatTensor(64, 4, 3, 3)
  local second_filterD16 = torch.FloatTensor(64, 64, 3, 3)
  -- first conv
  for i=1,64 do
    first_filterD16[i] = pa_G16[3][i]:clone()
  end
  for i =1,64 do
    first_filterD32[i] = image.scale(first_filterD16[i], 5, 5) --3*3 -> 5*5
    model_D32:get(9).weight[i] = first_filterD32[i]:clone()
  end
  for i=1,64 do
  ...
  -- second conv
  for i=1,64 do
    second_filterD16[i] = pa_D16[5][i]:clone()
  end
  for i =1,64 do
    second_filterD32[i] = image.scale(second_filterD16[i], 5, 5) --3*3 -> 5*5
    model_D32:get(11).weight[i] = second_filterD32[i]:clone()
  end
  --model_G32:get(10).bias = pa_G16[6]:clone() --bias
  for i=1,64 do
